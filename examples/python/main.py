...
def create_data_point(point):
    data = bytes('this is a python %s data point' % point)
    timestamp = int(time.time() * 1000)
    data_point = agent_pb2.Datapoint(
            stream="stream.001", data=data, timestamp=timestamp, type=agent_pb2.TEXT)
...
def http_post_data():
    data = {
        'stream': "stream.001",
        'timestamp':int(time.time() * 1000),
        'data': base64.b64encode(b'this is a python http posted data point'),
        'type': 'TEXT'
    }
